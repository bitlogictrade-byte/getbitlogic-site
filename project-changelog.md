...
- "플랜 업그레이드/구매" 버튼과 "플랜 관리" 버튼을 나란히(병렬) 배치
- "플랜 관리" 버튼 클릭 시 드로어(plan-manage-section) 토글 — 구독 해지 버튼 포함
- 구독 해지 버튼을 기존 `cancelSubRow`(info-row)에서 플랜 관리 드로어 내부로 이동
- pending 상태일 때 `planBtnRow` 전체 숨김 처리
- 구독 해지 완료 후 드로어 및 관리 버튼 자동 숨김
...
**수정 내용:**
- KG이니시스 빌링키 발급 요청에 `issueName: 'BitLogic 정기결제'` 추가
...
**수정 내용:**
- 다날(DANAL) 채널 테스트 결제 `payMethod`를 `EASY_PAY` → `CARD`로 변경
